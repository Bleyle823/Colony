...
// src/environment.ts
var circleEnvSchema = external_exports.object({
  CIRCLE_API_KEY: external_exports.string().min(1, "Circle API Key is required"),
  CIRCLE_ENTITY_SECRET: external_exports.string().min(1, "Circle Entity Secret is required"),
  EVM_PRIVATE_KEY: external_exports.string().optional(),
  // For BridgeKit
  SOLANA_PRIVATE_KEY: external_exports.string().optional()
  // For BridgeKit
});
async function validateCircleConfig(runtime) {
  try {
    const config = {
...
var crossChainTransferAction = {
  ...
  description: "Transfer USDC across blockchains using Circle BridgeKit (CCTP). Supports Ethereum, Solana, Arbitrum, Optimism, Base, Polygon, Avalanche.",
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const result = await kit.bridge({
        from: { adapter: fromAdapter, chain: getChainName(sourceChain) },
        to: { adapter: toAdapter, chain: getChainName(destChain) },
        amount
      });
...
var sendUsdcAction = {
  ...
  description: "Send USDC payment using Circle Developer-Controlled Wallets. Automatically selects a wallet with sufficient balance.",
  validate: async (runtime) => {
    try {
      await validateCircleConfig(runtime);
      return true;
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting SEND_USDC handler...");
    try {
      const config = await validateCircleConfig(runtime);
      ...
      const toMatch = text.match(/to (\w+)/i) || text.match(/address (\w+)/i);
      ...
      const chain = chainMatch ? chainMatch[1].toUpperCase() : "ETH";
      ...
      elizaLogger2.log("Transaction created:", transactionResponse);
      if (callback) {
        callback({
      ...
      elizaLogger2.error("Error in SEND_USDC:", error);
      if (callback) {
        callback({
...
var index_default = circlePlugin;
export {
  circlePlugin,
