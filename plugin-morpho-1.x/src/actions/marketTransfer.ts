...
- "Supply 100 AA_FalconXUSDC to mF-ONE/AA_FalconXUSDC market" → intent: supply, market: mF-ONE/AA_FalconXUSDC, assets: 100
</instructions>`;
}
...
export const marketTransferAction: Action = {
    ...
    try {
      ...
      switch (rawIntent) {
        case "supply":
          ...
          hashes = await service.supply(operationParams);
          break;

        case "supplycollateral":
          operationParams.assets = rawAssets;
          hashes = await service.supplyCollateral(operationParams);
          break;

        case "borrow":
          operationParams.assets = rawAssets;
          hashes = await service.borrow(operationParams);
          break;
        ...
        case "repay":
          ...
          hashes = await service.repay(operationParams);
          break;

        case "withdraw":
          operationParams.assets = rawAssets;
          hashes = await service.withdraw(operationParams);
          break;

        case "withdrawcollateral":
          operationParams.assets = rawAssets;
          hashes = await service.withdrawCollateral(operationParams);
          break;
  ...
  examples: [
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "Supply 100 AA_FalconXUSDC to mF-ONE/AA_FalconXUSDC market" },
      },
      {
        name: "{{agent}}",
        content: {
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "Use 50 mF-ONE as collateral to borrow 200 AA_FalconXUSDC" },
      },
      {
        name: "{{agent}}",
        content: {
          text: "I'll start by supplying the mF-ONE as collateral...",
          action: "MORPHO_MARKET_TRANSFER",
        },
      },
    ],
